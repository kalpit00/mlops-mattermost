...
``pyarrow`` or ``fastparquet``)::
    ...
    python -m mlops_data.pipelines.cli_jigsaw --source-dir ./path/to/csvs

**Run (library):** ``from mlops_data.pipelines import run_jigsaw_ingestion`` (requires
``PYTHONPATH`` including the repo root, or an installed package layout)::

    from pathlib import Path
    from mlops_data.pipelines import run_jigsaw_ingestion
