...
- **Python data pipelines**: `mlops_data/pipelines/*` (synthetic, dataset build, monitoring, promotion gate), plus optional Docker Compose stack
...
docker compose -f docker-compose-data.yml --profile mlops run --rm mlops-pipelines python -m mlops_data.pipelines.cli_monitoring --fail-on-breach
```
...
- **Python import errors**: re-run `python -m pip install -r mlops_data/pipelines/requirements.txt`.
