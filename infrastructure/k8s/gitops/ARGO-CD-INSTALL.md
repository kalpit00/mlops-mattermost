...
Use this as the **order of operations** after the cluster and platform stack are healthy. Aligns with the MLOps lab: *Terraform* → *bootstrap cluster* → *Argo CD* syncs from Git*.
...
- **App manifest root:** e.g. `infrastructure/k8s` with **Kustomize** overlays per env under `infrastructure/k8s/overlays/<env>` (once created).
...
- Keep `create-secrets.sh` as a **human bootstrap** only and mark Argo apps as *Depends* on that secret existing.
...
MLOps lab uses **Argo Workflows** for *pipelines* (retrain, batch). Install only after **Argo CD** and **Container Registry** are stable; it is a separate controller.

---

*See [docs/ARCHITECTURE-MASTER-PLAN.md](../../docs/ARCHITECTURE-MASTER-PLAN.md) for the full platform picture.*
